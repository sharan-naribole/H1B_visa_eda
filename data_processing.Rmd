...
Next, I rename the WORKSITE_STATE TO WORKSITE_STATE_ABB and merge it with the WORKSITE_CITY to form a new feature WORKSITE. This merge is performed because the same WORKSITE_CITY value might be present for multiple worksite states. For e.g., Houston is present in both Texas and California. To differentiate such locations, the merge is required.
...
h1b_df_tx$WORKSITE = mapply(site_merge,h1b_df_tx$WORKSITE_CITY,h1b_df_tx$WORKSITE_STATE_ABB)
...
```{r}
wrong_names = c("NEW YROK, NY", "SUUNYVALE, CA", "SAN FRANSISCO, CA")
...
with errors at a maximum of distance. This spell-correcter is inspired by the Python implementation described at http://norvig.com/spell-correct.html
...
function(split_left, split_right,i) {
...
edits2site <- function(site) { 
    ...
    edits1_cities = edits1site(site)
    return (unlist(sapply(edits1_cities, edits1site)))
}

function(site, site_hash) {
  # probability of site in our dataset
  return(site_hash[[site]])
}

site_spell_checker <- function(site,...) {
...
spell_correct <- function(site, site_spell_hash) {
  if(site_spell_hash$has_key(site)) {
    return(site_spell_hash[[site]])
  }
  return(site)
}
```
